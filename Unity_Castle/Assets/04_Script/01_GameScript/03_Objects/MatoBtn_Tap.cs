...
public class MatoBtn_Tap : TapCollider
{
    public int Idx;


    public MatoBtn_Judge JudgeClass;
    ...
    protected override void OnTap()
    {
        ...
        if (SaveLoadSystem.Instance.gameData.isOmake)
            return;
        if (SaveLoadSystem.Instance.gameData.isClearMatoBtn)
            return;

        AudioManager.Instance.SoundSE("Rolling");

        GameObject[] Btns = JudgeClass.Btns0;
        if(Idx == 1)
            Btns = JudgeClass.Btns1;
